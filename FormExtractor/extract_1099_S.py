...
def form_1099_S_extract(pdf_path, input_name,threshold):
    found_years = []
    data_list = []
    ...
    for page_index, img in enumerate(images):
        ...
        if '1099-S' in full_text:
            ...
            for i in range(n_boxes):
                ...
                if d['text'][i] == 'OMB' and d['left'][i] < 1570 and d['top'][i] < 150 and name_found:
                    ...
                    if years_list:
                        ...
                        for text in splited_text:
                            if 'OMB' in text:
                                year_area=True
                            elif '1099-S' in text or 'Transactions' in text:
                                break
                            if year_area==True:
                                year_text+=text+'\n'
                        years_list = re.findall(r'\b(20\d{2})\b', year_text)
                ...
                elif d['text'][i] == 'FILER’S' and d['left'][i] < 250 and d['top'][i] < 180:
                    (x, y, w, h) = (d['left'][i], d['top'][i], d['width'][i], d['height'][i])
                    x, y, w, h = x - 10, y + 50, w + 750, h + 150
                ...
                elif d['text'][i] == 'Gross' and d['left'][i] < 1300 and d['top'][i] < 250:
                    (x, y, w, h) = (d['left'][i], d['top'][i], d['width'][i], d['height'][i])
                    x, y, w, h = x - 40, y + 40, w + 290, h + 48
                    cv2.rectangle(img_bgr_resized, (x, y), (x + w, y + h), (0, 255, 0), 2)
                    crop_img = img_bgr_resized[y:y + h, x:x + w]
